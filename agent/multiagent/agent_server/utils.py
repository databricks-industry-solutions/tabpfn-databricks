...
def _sanitize_item(input_item: dict) -> dict:
    ...
    MCP tool calls can return items where ``output`` is a list instead of a
    plain string.  MLflow's Pydantic models expect a string, so we serialise
    any non-string values to JSON.
    """
